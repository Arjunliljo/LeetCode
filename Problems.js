...
var freqAlphabets = function (s) {
  const decrypt = (val) =>
    String.fromCharCode("a".charCodeAt(0) + Number(val) - 1);
...
var nextGreatestLetter = function (letters, target) {
  ...
  return (
    letters.find((char) => char.charCodeAt(0) > target.charCodeAt(0)) ||
    letters[0]
  );
};
...
var removeElement = function (nums, val) {
  const dupCount = nums.reduce(
    (acc, value) => (val === value ? acc + 1 : acc),
    0
  );
...
var productExceptSelf = function (nums) {
  ...
  const productFun = (i) =>
    nums.reduce((acc, value, j) => (i !== j ? acc * value : acc), 1);
...
var makeGood = function (s) {
  ...
  for (let char of s) {
    if (
      stack.length > 0 &&
      ((char === stack[stack.length - 1].toLowerCase() &&
        char.toUpperCase() === stack[stack.length - 1]) ||
        (char === stack[stack.length - 1].toUpperCase() &&
          char.toLowerCase() === stack[stack.length - 1]))
    ) {
      stack.pop();
    } else {
      stack.push(char);
...
var frequencySort = function (nums) {
  ...
  const frequency = (val) =>
    nums.reduce((acc, vals) => (val === vals ? acc + 1 : acc), 0);
...
var uniqueOccurrences = function (arr) {
  const count = (num) =>
    arr.reduce((acc, val) => (val === num ? acc + 1 : acc), 0);
...
var findLucky = function (arr) {
  const frequency = (num) =>
    arr.reduce((acc, val) => (num === val ? acc + 1 : acc), 0);
...
var lastStoneWeight = function (stones) {
  while (stones.length > 1) {
    ...
    else stones.push(max - secondMax);
  }
...
var repeatedNTimes = function (nums) {
  ...
  const count = (num) =>
    nums.reduce((acc, val) => (num === val ? acc + 1 : acc), 0);
...
var halvesAreAlike = function (s) {
  ...
  const vowelCount = (arr) =>
    arr.reduce((acc, char) => (vowels.includes(char) ? acc + 1 : acc), 0);
